...
class PppCoreTest(unittest.TestCase):
    ...
    def test_point_count_must_be_integer(self):
        ...
        with self.assertRaisesRegex(ValueError, "point_count must be an integer"):
            evaluate_case(case, point_count=True)
    ...
    def test_sample_case_derivations(self):
        ...
        self.assertEqual(result["engineering_review"]["statuses"], ["partial_source_safe_components"])
        self.assertNotIn("status", result["engineering_review"])
